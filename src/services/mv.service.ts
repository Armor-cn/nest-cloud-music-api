...
export class MvService extends BaseService {
  ...
  async mvAll(mvAllDto: MvAllDto) {
    ...
    const bashApi = encodeURI(
      this.cloudMusicApi +
        `mv/all?area=${data.area}&type=${data.type}&order=${data.order}&limit=${data.limit}&offset=${data.offset}`,
    );
  ...
  async mvFirst(mvFirstDto: MvFirstDto) {
    ...
    const bashApi = encodeURI(
      this.cloudMusicApi + `mv/first?area=${data.area}&limit=${data.limit}`,
    );
    const res = await this.httpService.post(bashApi).toPromise();
    return await res.data;
  }
  ...
  async mvExclusiveRcmd(mvExclusiveRcmd: MvExclusiveRcmd) {
    ...
    const bashApi =
      this.cloudMusicApi +
      `mv/exclusive/rcmd?limit=${data.limit}&offset=${data.offset}`;
    const res = await this.httpService.post(bashApi).toPromise();
    return await res.data;
  }
