...
export class AlbumService extends BaseService {
  ...
  async albumDetailDynamic(albumDetailDynamicDto: AlbumDetailDynamicDto) {
    ...
    const bashApi =
      this.cloudMusicApi +
      `album/detail/dynamic?id=${albumDetailDynamicDto.id}`;
    const res = await this.httpService
      .post(bashApi, albumDetailDynamicDto)
      .toPromise();
    return await res.data;
  }
  ...
  async albumSub(albumSubDto: AlbumSubDto) {
    ...
    const bashApi =
      this.cloudMusicApi + `album/sub?id=${albumSubDto.id}&t=${albumSubDto.t}`;
