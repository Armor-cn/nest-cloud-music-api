...
export class AlbumController {
    ...
    @ApiOperation({ summary: '获取专辑内容', description: '说明 : 调用此接口 , 传入专辑 id, 可获得专辑内容 实例： /album?id=32311' })
    @Get()
    async album(@Query() albumDto: AlbumDto) {
        return await this.albumService.album(albumDto);
    }

    @ApiOperation({ summary: '专辑动态信息', description: '说明 : 调用此接口 , 传入专辑 id, 可获得专辑动态信息,如是否收藏,收藏数,评论数,分享数, 实例：/album/detail/dynamic?id=32311' })
    @Get('/detail/dynamic')
   ...
   */
    ...
    @ApiOperation({ summary: '获取已收藏专辑列表', description: '说明 : 调用此接口 , 可获得已收藏专辑列表,（需要登录） 实例： /album/sublist ' })
    @Get('/sublist')
    async albumSubList(@Query() albumSubListDto: AlbumSubListDto) {
        return await this.albumService.albumSubList(albumSubListDto);
    }

    @ApiOperation({ summary: '最新专辑', description: '说明 : 调用此接口 ，获取云音乐首页新碟上架数据 实例： /album/newest' })
    @Get('/newest')
    async  albumNewest() {
