...
export class TopController {
  ...
  @ApiOperation({
    summary: '新歌速递',
    description: '调用此接口, 可获取新歌速递，实例: /top/song?type=0',
  })
  ...
  @ApiOperation({
    summary: '新碟上架',
    description:
      '说明 : 调用此接口 , 可获取新碟上架列表, 如需具体音乐信息需要调用获取专辑列表接 口 /album , 然后传入 id, 如 /album?id=32311&limit=30 ,本接口实例： /top/album?offset=0&limit=30',
  })
  @Get('/album')
  ...
  @ApiOperation({
    summary: '热门歌手',
    description:
      ' 说明 : 调用此接口 , 可获取热门歌手数据 实例： /top/artists?offset=0&limit=30',
  })
  @Get('/artists')
  ...
  @ApiOperation({
    summary: 'mv 排行',
    description: '说明 : 调用此接口 , 可获取 mv 排行 实例：/top/mv?limit=10',
  })
  @Get('/mv')
