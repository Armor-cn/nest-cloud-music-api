...
export class MvController {
  ...
  @ApiOperation({
    summary: '全部 mv',
    description:
      '说明 : 调用此接口 , 可获取全部 mv 实例：  /mv/all?area=韩国&order=最热&limit=1',
  })
  @Get('/all')
  ...
  @ApiOperation({
    summary: '最新 mv',
    description: '说明 : 调用此接口 , 可获取最新 mv 实例: /mv/first?limit=10',
  })
  @Get('/first')
  ...
  @ApiOperation({
    summary: '网易出品mv',
    description:
      '说明 : 调用此接口 , 可获取网易出品 mv 实例：/mv/exclusive/rcmd?limit=10 ',
  })
  @Get('/exclusive/rcmd')
  ...
  @ApiOperation({
    summary: '获取 mv 数据',
    description:
      '说明 : 调用此接口 , 传入 mvid ( 在搜索音乐的时候传 type=1004 获得 ) , 可获取对应 MV 数据 , 数据包含 mv 名字 , 歌手 , 发布时间 , mv 视频地址等数据 , 其中 mv 视频 网易做了防盗链处理 , 可能不能直接播放 , 需要播放的话需要调用  mv 地址 接口 实例：/top/mv?limit=10',
  })
  @Get('/detail')
  ...
  @ApiOperation({
    summary: 'mv 地址',
    description:
      '说明 : 调用此接口 , 传入 mv id,可获取 mv 播放地址 实例：/mv/url?id=5436712',
  })
  @Get('/url')
