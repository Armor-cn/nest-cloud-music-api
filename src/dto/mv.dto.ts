...
export class MvAllDto {
    @ApiProperty({ description: 'area: 地区,可选值为全部,内地,港台,欧美,日本,韩国,不填则为全部 ', required: false, default: '' })
    area: string;
    @ApiProperty({ description: 'type: 类型,可选值为全部,官方版,原生,现场版,网易出品,不填则为全部', default: '', required: false })
    type: string;
    @ApiProperty({ description: '排序,可选值为上升最快,最热,最新,不填则为上升最快', default: '', required: false })
    order: string;
    @ApiProperty({ description: '取出数量 , 默认为 50', required: false, default: 50 })
    limit: number;
