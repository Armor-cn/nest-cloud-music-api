...
export class BaseQueryDto {
    ...
    @ApiProperty({ description: '通用分页-每页数量', required: false, default: 25 })
    @IsNumber()
    @Type(() => Number)
    readonly pageSize: number = 25;

    @ApiProperty({ description: '通用分页-分页模式, 非特殊情况不用传递  默认分页  传递值为no表示不分页' })
    @IsIn(['no'])
    readonly pageMode?: string;

    @ApiProperty({ description: `通用排序, 例如[['createdAt', 'desc'],['name', 'asc']]`, type: [String], example: [['name', 'desc']] })
...
export class BannerDto {
    @ApiProperty({ description: 'type:资源类型,对应以下类型,默认为 0 即PC ', default: 0, required: false })
    type: number;
}

export class SongDetailsDto {
    @ApiProperty({ description: '必选参数 : ids: 音乐 id, 如 ids=347230 ', required: true })
    ids: string;
}

export class DailySigninDto {
    @ApiProperty({ description: ' type: 签到类型 , 默认 0, 其中 0 为安卓端签到 ,1 为 web/PC 签到', default: 0, required: false })
    type: number;
}
