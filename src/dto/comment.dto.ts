...
export class CommentMusicDto {
  ...
  id: string;
  @ApiProperty({
    description: 'limit: 取出评论数量 , 默认为 20',
    default: 20,
    required: false,
  })
  limit: number;
...
export class CommentHotDto {
  ...
  @ApiProperty({
    description:
      '必传参数 : tpye: 数字 , 资源类型 , 对应歌曲 , mv, 专辑 , 歌单 , 电台, 视频对应以下类型: 0: 歌曲, 1: mv, 2: 歌单, 3: 专辑, 4: 电台, 5: 视频',
    required: true,
  })
  type: number;
  @ApiProperty({
...
export class CommentLikeDto {
  @ApiProperty({
    description: '必传参数 : 资源 id, 如歌曲 id,mv id',
    required: true,
  })
  id: string;
  ...
  cid: string;
  @ApiProperty({
    description: ': 是否点赞 ,1 为点赞 ,0 为取消点赞',
    required: true,
  })
  t: number;
  @ApiProperty({
    description:
      '必传参数 : tpye: 数字 , 资源类型 , 对应歌曲 , mv, 专辑 , 歌单 , 电台, 视频对应以下类型 0: 歌曲, 1: mv, 2: 歌单, 3: 专辑, 4: 电台, 5: 视频, 6: 动态',
    required: true,
  })
  type: number;
}
